...
layui.define(["element", "jquery"], function (exports) {
    ...
    Tab.prototype.tabAdd = function (_this) {
        ...
        if (_this.attr("target") == "_blank") {
            ...
            if (that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0) {
                ...
                //当前窗口内容
                var curmenu = {
                    "icon": _this.find("i.iconfont").attr("data-icon") != undefined ? _this.find("i.iconfont").attr("data-icon") :
    ...
    $("body").on("click", ".top_tab li i.layui-tab-close", function () {
        ...
        var curmenu = window.sessionStorage.getItem("curmenu") == "undefined" ? undefined : window.sessionStorage.getItem(
            "curmenu") == "" ? '' : JSON.parse(window.sessionStorage.getItem("curmenu"));
